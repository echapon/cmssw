...
private:
    ...
    virtual void fillCentrality(const edm::Event& iEvent);
...
NtupleMaker::NtupleMaker(const edm::ParameterSet& iConfig):
    ...
    putCentrality                  (iConfig.getUntrackedParameter<bool>("putCentrality")),    
...
void NtupleMaker::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
   ...
   // }
    ...
    if(putCentrality) fillCentrality(iEvent);
        ...
        for( unsigned j = 0; j != ntrigs; j++ ) {
            if( _trigName.find(trigs[j].substr(0,trigs[j].find("*"))) != std::string::npos ) {
                ...
                if( ResultHandle.product()->accept(i) ) {
                    // cout<<"Fired: "<<trigger_.name << " " << trigs[j].substr(0,trigs[j].find("*")) <<endl;
...
// ------------ method fills ntuple with the HI event info
void NtupleMaker::fillCentrality(const edm::Event& iEvent) {
   ...
   event_.hiNpix = centrality->multiplicityPixel();
   event_.hiNpixelTracks = centrality->NpixelTracks();
   event_.hiNtracks = centrality->Ntracks();
   event_.hiNtracksPtCut = centrality->NtracksPtCut();
   event_.hiNtracksEtaCut = centrality->NtracksEtaCut();
   event_.hiNtracksEtaPtCut = centrality->NtracksEtaPtCut();

   event_.hiHF = centrality->EtHFtowerSum();
   event_.hiHFplus = centrality->EtHFtowerSumPlus();
   event_.hiHFminus = centrality->EtHFtowerSumMinus();
   event_.hiHFplusEta4 = centrality->EtHFtruncatedPlus();
   event_.hiHFminusEta4 = centrality->EtHFtruncatedMinus();
   event_.hiHFhit = centrality->EtHFhitSum();
   event_.hiHFhitPlus = centrality->EtHFhitSumPlus();
   event_.hiHFhitMinus = centrality->EtHFhitSumMinus();

   event_.hiZDC = centrality->zdcSum();
   event_.hiZDCplus = centrality->zdcSumPlus();
   event_.hiZDCminus = centrality->zdcSumMinus();

   event_.hiEEplus = centrality->EtEESumPlus();
   event_.hiEEminus = centrality->EtEESumMinus();
   event_.hiEE = centrality->EtEESum();
   event_.hiEB = centrality->EtEBSum();
   event_.hiET = centrality->EtMidRapiditySum();
}
